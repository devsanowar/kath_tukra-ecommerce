...
@section('title', 'All Field of Cost')
        ...
        <div class="row clearfix">
            <div class="col-lg-12 col-md-12 col-sm-12 mt-4">
                <div class="card">
                            ...
                            <tbody>
                                @foreach ($fields as $key => $field)
                                    <tr>
                                        ...
                                        <td class="category-name">{{ $field->field_of_cost }}</td>
                                        <td class="category-slug">{{ $field->field_slug }}</td>

                                        <td>
                                            <button data-id="{{ $field->id }}"
                                                class="btn btn-sm status-toggle-btn {{ $field->is_active ? 'btn-success' : 'btn-danger' }}">
                                                {{ $field->is_active ? 'Active' : 'DeActive' }}
                                            </button>
                                        </td>
                                        <td>
                                            <a href="javascript:void(0)" class="btn btn-warning btn-sm editcategory"
                                                data-id="{{ $field->id }}" data-name="{{ $field->field_of_cost }}"
                                                data-status="{{ $field->is_active }}">
                                                <i class="material-icons text-white">edit</i>
                                            </a>

                                            <form id="delete-form-{{ $field->id }}"
                                                  action="{{ route('field_of_cost.destroy', $field->id) }}"
                                                  method="POST"
                                                  class="d-inline-block">
                                                @csrf
                                                @method('DELETE')
                                                <button type="button" data-id="{{ $field->id }}" class="btn btn-danger btn-sm show_confirm">
                    ...
                    @include('admin.layouts.pages.field-of-cost.create')

                    <!-- Edit category Modal -->
                    @include('admin.layouts.pages.field-of-cost.edit')
...
@push('scripts')
    ...
    <script>
        const categoryStatusRoute = "{{ route('field_of_cost.status') }}";
